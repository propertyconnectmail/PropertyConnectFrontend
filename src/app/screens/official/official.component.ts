...
export class OfficialComponent {
  constructor(private router: Router, private officialService: OfficialService, private toastServie: ToastService) { }
    ...
    ngOnInit(): void {
      ...
      this.officialService.getAllOfficials().subscribe(
        (res: any) => {
          console.log(res);
          if (res && res.length > 0) {
    ...
    add() {
      this.router.navigate(['/official', 'add']);
    }
  
    view(id: string) {
      this.router.navigate(['/official', 'view', id]);
    }
  
    edit(id: string) {
      this.router.navigate(['/official', 'edit', id]);
    }
  
    delete(id: string) {
      this.router.navigate(['/official', 'delete', id]);
    }
}
